...
const CategoryList = () => {
  ...
  return (
    <div className={`${data.categoryListDropdown ? "" : "hidden"} my-4`}>
      <hr />
      <div className="py-2 grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 max-w-6xl mx-auto px-4">
            ...
            return (
              <Fragment key={index}>
                <div
                  ...
                  className="col-span-1 flex flex-col items-center justify-center space-y-2 cursor-pointer hover:bg-gray-50 p-2 rounded-lg transition"
                >
                  <img
                    ...
                    alt="pic"
                  />
                  <div className="font-medium text-sm text-center">{item.cName}</div>
                </div>
              </Fragment>
            );
          })
        ) : (
          <div className="col-span-3 md:col-span-4 lg:col-span-6 text-xl text-center my-4">No Category</div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className={`${data.filterListDropdown ? "" : "hidden"} my-4`}>
      ...
      <div className="w-full flex flex-col">
        <div className="font-medium py-2">Khoảng giá</div>
        <div className="flex justify-between items-center">
          <div className="flex flex-col space-y-2  w-2/3 lg:w-2/4">
            <label htmlFor="points" className="text-sm">
              Giá:{" "}
              <span className="font-semibold text-yellow-700">{range}.000 VNĐ</span>{" "}
            </label>
            <input
              value={range}
              className="slider"
