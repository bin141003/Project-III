...
const OrderMenu = (props) => {
  ...
  const handleSearch = (searchValue) => {
    const sourceOrders = data.allOrders && data.allOrders.length > 0 
      ? data.allOrders 
      : data.orders;

    if (!searchValue || searchValue.trim() === "") {
      // search rỗng, hiển thị tất cả
      dispatch({ type: "searchOrders", payload: sourceOrders });
    } else {
      // lọc theo ID
      const filtered = (sourceOrders || []).filter(order => 
        order?.transactionId?.toLowerCase().includes(searchValue.toLowerCase())
      );
      dispatch({ type: "searchOrders", payload: filtered });
    }
  };
